...
fn is_low_point(grid: &Vec<Vec<u32>>, row: usize, col: usize) -> bool {
    let mut result: bool = grid.len() > 0
        && grid.first().unwrap().len() > 0
        && row < grid.len()
    ...
    if result && col + 1 < grid.first().unwrap().len() {
        // check right
        result &= grid[row][col] < grid[row][col + 1];
    }
    if result && row + 1 < grid.len() {
        // check down
        result &= grid[row][col] < grid[row + 1][col];
    }
...
fn collect_basin(
    ...
    // down
    if row + 1 < grid.len() && !basin.contains(&(row + 1, col)) && grid[row + 1][col] != 9 {
    ...
    // up
    if row >= 1 && !basin.contains(&(row - 1, col)) && grid[row - 1][col] != 9 {
        to_collect.push((row - 1, col));
    }
    // left
    if col >= 1 && !basin.contains(&(row, col - 1)) && grid[row][col - 1] != 9 {
        to_collect.push((row, col - 1));
    }

    to_collect
        .iter()
        .for_each(|(newrow, newcol)| collect_basin(&grid, *newrow, *newcol, basin));
}
